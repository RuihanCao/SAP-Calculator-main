...
describe('App integration overrides', () => {
  describe('facing', () => {
    /**
...
/**
 * Every number below is read off a reference frame, and the frame is
 ...
 */
describe('measured against the reference frames', () => {
  const styles = readFileSync(
    path.join(stageDir, 'battle-animation-stage.component.scss'),
    'utf8',
  );
  ...
  describe('the pet card, items 6, 7 and 8', () => {
    ...
    it('builds the badges as a numeral inside a rock and inside a heart', () => {
      ...
      // The shapes are the pack's own text-map sprites, which arrive
    ...
    it('draws the level plaque as a wooden two-lobed sign', () => {
      // Cut whole out of the client rather than drawn. Everything except
      // the numeral is in the cut, cap and "Lvl" included, because only the
      // numeral changes; reading the cap as the lettering's own keyline and
      // leaving it out came back a third short.
      const plaque = styles.slice(styles.indexOf('.anim-pet-level {'));
      // The plaque is one ripped sprite drawn whole, LevelMap_0/1/2,
      ...
      // The numeral is part of the sprite, so there is no numeral rule
  ...
  describe('fainting and the corpse, items 2 and 10', () => {
    ...
    /**
     * The pet that has just died is *not* washed out: it stands in its
    ...
    /**
     * The lethal hit leaves the body in place under a crossed bandage with
     * its real, possibly negative, health showing, and only then does the
     * corpse launch. The
    ...
    /**
     * The trail was built the right way at half the size, which read as
     * thinner than the client's. Measured on f02 t=31.88 at native
     * resolution: the band behind
    ...
    it('lays the trail as a chain of clouds rather than one wisp', () => {
      ...
      const puff = styles.slice(styles.indexOf('.anim-corpse-puff {'));
      // The same cloud a summon uses, and it is the client's own puff cut
      // out of a reference frame rather than a stack of drawn circles. The
      // client's trail is flat, stroke-free grey circles rather than one soft
      // puff texture, measured on the close-up against f11b.
     ...
     * because an early note asked for the opposite, and the frame decides.
     */
    ...
    /**
     * Traced frame by frame off the reference clips: on f03
     ...
     * against 35% of the height. An earlier 22 across and 56 up is the steep
     * arc that read as unlike the client's.
     */
  ...
  describe('the control bar, item 13', () => {
    ...
    it('draws every glyph rather than borrowing a font character', () => {
      ...
      // The glyphs are the client's own printing, lifted off its bar,
    ...
    it('sizes the tiles and the row off the reference bar', () => {
      ...
      // Re-measured at a 720 play area: 50.0 by 58.7 css with a
  ...
  describe('the end screen veil, items 5 and 17', () => {
     ...
     * f_01926_0054246, so 0.633 s. A linear 0.7 s crossed the same two
    ...
    it('sets the caption where the reference sets it', () => {
      ...
      // Measured on the close-up, the client's caption goes straight
  ...
  describe('the damage numeral, item 11', () => {
     ...
     * face, red with a heavy black outline. An earlier numeral was 1.9 scale
     ...
     * the red muddy brown on the first recording.
     */
    it('is the game face, red, black-outlined, at the reference size', () => {
      ...
      // Re-read by masking pure red on f02's "8" frame by
  ...
  describe('the ability toast, item 14', () => {
    ...
    it('is sized and anchored off the reference plate', () => {
      ...
      // The toast wears the cut plate, and so do the trumpet counter
      // and the stat popups, which is what makes them read as one layer instead
      // of three separately invented white boxes. No drop shadow either, the
      // reference toast does not have one.
      expect(banner).toMatch(/@include game-plate/);
      const mixin = styles.indexOf('@mixin game-plate');
      expect(styles.slice(mixin, mixin + 400)).toContain('plate-name.png');
      expect(styles.slice(styles.indexOf('.anim-counter {'))).toContain('@include game-plate');
    ...
    it('puts a rock inline for damage rather than a grey blob', () => {
      ...
      // The inline damage glyph is the pack's own rock sprite, the one
    ...
    /**
     * The snipe had no throw to it. Three things were wrong and all three are
     * measurements: the object was a
     ...
     */
    /**
     * Off the buff close-up: the client sets a white plus and
